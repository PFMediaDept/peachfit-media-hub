...
export default function PipelineBoard() {
  ...
  async function fetchStatuses() {
    const { data } = await supabase.from('pipeline_statuses').select('*').eq('branch_slug', slug).eq('is_sob', false).order('sort_order')
    setStatuses(data || [])
  }
  async function fetchTasks() {
    const { data, error } = await supabase.from('pipeline_tasks').select('*').eq('branch_slug', slug).eq('is_sob', false).order('sort_order')
